  ...
  if (props.color) {
    return (
      <Link href={props.route} className="max-w-[250px] max-h-[60px] flex items-center justify-center rounded mt-[20px]">
              <button className={`bg-${props.color}-500 py-2 px-6 rounded text-white text-[20px] max-w-[250px] cursor-pointer transition hover:bg-${props.color}-400 dark:font-bold`}>{props.content}</button>
      </Link>
    );
  }
